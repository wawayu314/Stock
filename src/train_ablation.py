...
import time # 新增导入，用于记录训练时间

# ====================================================================================
# --- 消融实验配置 ---
# 请每次只将一个标志位设为True，以进行独立的消融实验
# ====================================================================================
ABLATION_NO_SPARSE = True # 禁用稀疏注意力，使用全连接注意力
ABLATION_NO_FUSION = False # 禁用多尺度融合
ABLATION_NO_TRIU   = False  # 禁用TriU网络，使用标准线性层

# 自动生成模型名称和Excel文件路径
ablation_name_parts = []
if ABLATION_NO_SPARSE:
    ablation_name_parts.append("NoSparse")
if ABLATION_NO_FUSION:
    ablation_name_parts.append("NoFusion")
if ABLATION_NO_TRIU:
    ablation_name_parts.append("NoTriU")

if len(ablation_name_parts) > 1:
    print("❌ 错误：一次只能进行一种消融实验。请只将一个ABLATION标志位设为True。")
    exit()
elif len(ablation_name_parts) == 0:
    MODEL_VARIANT_NAME = "FullModel" # 完整模型
    EXCEL_FILE_PATH = '../training_results.xlsx'
else:
    MODEL_VARIANT_NAME = f"Ablation_{'_'.join(ablation_name_parts)}"
    EXCEL_FILE_PATH = '../training_results_ablation.xlsx'

print("="*60)
print(f"🚀 开始运行实验: {MODEL_VARIANT_NAME}")
print("="*60)
# ====================================================================================
...
attention_dropout = 0.25 # 略微降低正则化
weight_decay = 0.0011 # 略微降低正则化
...
# 当禁用多尺度融合时，scale_factor会在模型内部被设为1，这里计算保持一致
effective_scale_factor = 1 if ABLATION_NO_FUSION else scale_factor
num_conv_scales_to_add = max(0, effective_scale_factor - 1)
...
model = StockPredict(
    ...
    # --- 传递消融实验标志 ---
    use_sparse_attention=(not ABLATION_NO_SPARSE),
    use_multiscale_fusion=(not ABLATION_NO_FUSION),
    use_triu_network=(not ABLATION_NO_TRIU)
).to(device)
...
if ticker_list:
    try:
        ...
        # 获取稀疏度统计
        if not ABLATION_NO_SPARSE and model.stock_attention_mixer.sparse_attention.industry_mask is not None:
...
# 开始训练循环
total_training_time = 0.0 # 用于累加总训练时长
for epoch in range(epochs):
    ...
    # 学习率调度器步进 (现在支持预热)
    print(f"Epoch {epoch+1}/{epochs} - Current LR: {optimizer.param_groups[0]['lr']:.2e}")
    scheduler.step()
...
if best_valid_perf and best_test_perf and best_epoch_num > 0:
    model_name_for_excel = f"StockPredict_{MODEL_VARIANT_NAME}"
    
    # 计算平均epoch耗时
    avg_epoch_time = total_training_time / epochs if epochs > 0 else 0
    
    # 获取稀疏注意力统计信息
    sparse_attention_info = {
        'enabled': (not ABLATION_NO_SPARSE),
        'sparsity': 0.0,
