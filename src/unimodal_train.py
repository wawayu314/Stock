...
特点：
1. 提供两种单模态基准：价格-only 和 情绪-only
2. 用于与门控多模态融合模型进行对比实验
...
# 选择模型类型: "price_only" 或 "sentiment_only"
MODEL_TYPE = "sentiment_only"  # 可改为 "sentiment_only"
...
def compute_price_norm_params(train_samples: List[Dict]) -> Tuple[np.ndarray, np.ndarray]:
    """计算价格特征的标准化参数。"""
    ...
    print("✅ 已根据训练集计算价格特征标准化参数")
    return price_mean.astype(np.float32), price_std.astype(np.float32)


def compute_sentiment_norm_params(train_samples: List[Dict]) -> Tuple[np.ndarray, np.ndarray]:
    """计算情绪特征的标准化参数。"""
    ...
    print("✅ 已根据训练集计算情绪特征标准化参数")
    return senti_mean.astype(np.float32), senti_std.astype(np.float32)
...
class SentimentOnlyDataset(Dataset):
    """情绪-only 数据集。"""
...
class PriceOnlyModel(nn.Module):
    ...
    def __init__(
        ...
        super(PriceOnlyModel, self).__init__()
        ...
        self.output_fc = nn.Sequential(
            nn.Linear(price_encoder_hidden, price_encoder_hidden),
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(price_encoder_hidden, 1)
        )

    def forward(self, price_x: torch.Tensor) -> torch.Tensor:
        """price_x: (batch, seq_len, price_input_dim)"""
        price_feat = self.price_encoder(price_x)
        output = self.output_fc(price_feat)
        return output


class SentimentOnlyModel(nn.Module):
    """情绪-only 模型。"""
    ...
    def __init__(
        ...
        super(SentimentOnlyModel, self).__init__()
        ...
        self.output_fc = nn.Sequential(
            nn.Linear(sentiment_encoder_hidden, sentiment_encoder_hidden),
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(sentiment_encoder_hidden, 1)
        )

    def forward(self, senti_x: torch.Tensor) -> torch.Tensor:
        """senti_x: (batch, seq_len, sentiment_input_dim)"""
        senti_feat = self.sentiment_encoder(senti_x)
        output = self.output_fc(senti_feat)
        return output


# ===================== 评估函数 =====================

def evaluate_price_only(
    model: nn.Module,
    ...
    with torch.no_grad():
        for batch in dataloader:
            price_x, y, stock_idx, time_idx = batch
...
def evaluate_sentiment_only(
    ...
    with torch.no_grad():
        for batch in dataloader:
            senti_x, y, stock_idx, time_idx = batch
            senti_x = senti_x.to(device)
            y = y.to(device)
            out = model(senti_x)
            loss = criterion(out, y)
            batch_size = senti_x.size(0)
            total_loss += loss.item() * batch_size
            total_count += batch_size
            preds = out.squeeze(1).cpu().numpy()
            gts = y.squeeze(1).cpu().numpy()
...
def main():
    ...
    print(f"💻 使用设备: {device}")
    print(f"🧪 市场: {MARKET_NAME}, 单模态基准模型: {MODEL_TYPE}")
    print("=" * 60)
    ...
    if MODEL_TYPE == "price_only":
        price_mean, price_std = compute_price_norm_params(train_samples)
    else:
        senti_mean, senti_std = compute_sentiment_norm_params(train_samples)

    # 7. 构建数据集
    if MODEL_TYPE == "price_only":
        train_dataset = PriceOnlyDataset(train_samples, price_mean, price_std)
        valid_dataset = PriceOnlyDataset(valid_samples, price_mean, price_std)
        test_dataset = PriceOnlyDataset(test_samples, price_mean, price_std)
    else:
        train_dataset = SentimentOnlyDataset(train_samples, senti_mean, senti_std)
        valid_dataset = SentimentOnlyDataset(valid_samples, senti_mean, senti_std)
        test_dataset = SentimentOnlyDataset(test_samples, senti_mean, senti_std)
    ...
    if MODEL_TYPE == "price_only":
        print(f"📊 价格特征维度: {price_input_dim}")
        model = PriceOnlyModel(
        ...
        print(f"📊 情绪特征维度: {sentiment_input_dim}")
    ...
    for epoch in range(EPOCHS):
        ...
        # 验证与测试评估
        if MODEL_TYPE == "price_only":
            valid_perf = evaluate_price_only(
            ...
            valid_perf = evaluate_sentiment_only(
                model, valid_loader, device,
                stock_num=len(tickers),
                start_day_index=valid_index,
                num_days_in_split=test_index - valid_index,
            ...
            test_perf = evaluate_sentiment_only(
                model, test_loader, device,
                stock_num=len(tickers),
                start_day_index=test_index,
                num_days_in_split=num_dates - test_index,
    ...
    if best_valid_perf and best_test_perf and best_epoch > 0:
        model_name_for_excel = "PriceOnly" if MODEL_TYPE == "price_only" else "SentimentOnly"
        ...
        header = ["Model", "Dataset", "Best Epoch",
                  ...
                  "Encoder Hidden", "Encoder Layers", "Dropout",
                  "Batch Size", "Total Epochs",
