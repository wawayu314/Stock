...
def process_bad_data(base_path):
    """
    加载 "坏" 的 NASDAQ 数据, 对其进行清洗和归一化,
    然后保存为一套新的、干净的 .pkl 文件。
    """
    print("--- 开始处理 '坏' NASDAQ 数据 ---")
    ...
    ticker_path = os.path.join(base_path, 'nasdaq_tickers.txt') # 读取原始 ticker 文件
    ...
    # 新增的输出文件
    output_nasdaq_npy_path = os.path.join(base_path, 'NASDAQ.npy')
    output_baseline_npy_path = os.path.join(base_path, 'baseline_data_nasdaq.npy')
    output_ticker_csv_path = os.path.join(base_path, 'nasdaq_ticker.csv')
    ...
    try:
        ...
        # 1. 保存 NASDAQ.npy
        np.save(output_nasdaq_npy_path, eod_data_normalized)

        # 2. 生成并保存 baseline_data_nasdaq.npy
        baseline_data = np.mean(eod_data_normalized, axis=0)
        np.save(output_baseline_npy_path, baseline_data)
        
        # 3. 保存 nasdaq_ticker.csv
        ...
        print(f"  --- 新增文件 ---")
        print(f"  - {os.path.basename(output_nasdaq_npy_path)} (numpy array)")
...
if __name__ == "__main__":
    process_bad_data(base_path='../dataset/NASDAQ/') 