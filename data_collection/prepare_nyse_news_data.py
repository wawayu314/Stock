...
def get_nyse_major_tickers():
    """获取NYSE股票列表 - 从数据集文件中读取"""
    # 读取数据集中的NYSE股票列表
    OUTPUT_BASE_DIR = '../dataset'
    nyse_file_path = os.path.join(OUTPUT_BASE_DIR, 'NYSE', 'nyse_tickers.txt')
    
    if not os.path.exists(nyse_file_path):
        print(f"错误：未找到NYSE股票列表文件 {nyse_file_path}")
        return []
    
    try:
        # 读取txt文件
        with open(nyse_file_path, 'r') as f:
            nyse_tickers = [line.strip() for line in f.readlines() if line.strip()]
        
        print(f"从数据集文件读取到 {len(nyse_tickers)} 只NYSE股票")
        return nyse_tickers
        
    except Exception as e:
        print(f"读取NYSE数据文件出错: {e}")
        return []

def download_news_batch(ticker_list, start_date, end_date, news_dir, batch_size=200):
    """批量下载新闻情绪数据 - 升级版API优化"""
    print(f"\n{'='*60}")
    print(f"开始下载NYSE新闻情绪数据 (升级版API - 高速模式)")
    ...
    if all_daily_sentiment:
        ...
        combined_filepath = os.path.join(news_dir, 'nyse_combined_daily_sentiment.csv')
        combined_sentiment.to_csv(combined_filepath, index=False)
        
        print(f"\n✅ NYSE新闻数据收集完成！(升级版API高速模式)")
...
def main():
    ...
    print(f"🏛️ NYSE新闻情绪数据收集 (升级版API - 高速模式)")
    print(f"新闻情绪数据将保存到: {os.path.abspath(NEWS_DIR)}")
    print(f"数据时间范围: {START_DATE_STR} 到 {END_DATE_STR}")
    print(f"✅ 升级版API：无限制调用 + 高速收集优化")

    # 1. 获取NYSE股票列表
    nyse_tickers = get_nyse_major_tickers()
    
    if not nyse_tickers:
        print("错误：无法获取NYSE股票列表")
        return
    
    # 保存股票列表
    ticker_list_path = os.path.join(DATA_DIR, 'nyse_tickers.txt')
    with open(ticker_list_path, 'w') as f:
        for ticker in nyse_tickers:
            f.write(f"{ticker}\n")
    
    print(f"📊 数据集统计:")
    print(f"  - NYSE股票总数: {len(nyse_tickers)} 只")
    ...
    # 显示前10只和最后10只股票作为预览
    print(f"\n📝 股票列表预览:")
    print(f"前10只: {', '.join(nyse_tickers[:10])}")
    print(f"最后10只: {', '.join(nyse_tickers[-10:])}")
    
    confirm = input(f"\n确认开始收集全部 {len(nyse_tickers)} 只NYSE股票的新闻数据？(y/n): ").strip().lower()
    if confirm != 'y':
        print("取消收集")
        return
    
    process_tickers = nyse_tickers
    ...
    if combined_sentiment is not None:
        # 3. 保存成功处理的股票列表
        success_ticker_path = os.path.join(NEWS_DIR, 'nyse_news_tickers.txt')
        with open(success_ticker_path, 'w') as f:
            for ticker in successful_tickers:
                f.write(f"{ticker}\n")
        
        print(f"\n📊 NYSE新闻数据收集总结 (升级版高速模式):")
        print(f"  - 数据集总股票数: {len(nyse_tickers)}")
        print(f"  - 成功处理股票数: {len(successful_tickers)}")
        print(f"  - 成功率: {len(successful_tickers)/len(nyse_tickers)*100:.1f}%")
        print(f"  - 数据保存位置: {NEWS_DIR}")
        print(f"  - 股票列表保存: {success_ticker_path}")
        print(f"  - 合并数据文件: nyse_combined_daily_sentiment.csv")
        print(f"  - 收集效率: 升级版API高速优化")
        
    else:
        print("❌ NYSE新闻情绪数据收集失败")
