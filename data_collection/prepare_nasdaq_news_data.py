...
def download_news_batch(ticker_list, start_date, end_date, news_dir, batch_size=200):
    """批量下载新闻情绪数据 - 升级版API优化"""
    print(f"\n{'='*60}")
    print(f"开始下载NASDAQ新闻情绪数据 (升级版API - 高速模式)")
    ...
    if all_daily_sentiment:
        ...
        combined_filepath = os.path.join(news_dir, 'nasdaq_combined_daily_sentiment.csv')
        combined_sentiment.to_csv(combined_filepath, index=False)
        
        print(f"\n✅ NASDAQ新闻数据收集完成！(升级版API高速模式)")
...
def main():
    ...
    MARKET_NAME = 'NASDAQ'
    PRICE_DATA_DIR = os.path.join(OUTPUT_BASE_DIR, MARKET_NAME)
    NEWS_DIR = os.path.join(OUTPUT_BASE_DIR, MARKET_NAME, 'news_sentiment')
    ...
    print(f"🏢 NASDAQ新闻情绪数据收集 (升级版API - 高速模式)")
    print(f"新闻情绪数据将保存到: {os.path.abspath(NEWS_DIR)}")
    print(f"数据时间范围: {START_DATE_STR} 到 {END_DATE_STR}")
    print(f"✅ 升级版API：无限制调用 + 高速收集优化")

    # 1. 读取NASDAQ股票列表 - 读取数据集中的所有股票
    ticker_list_path = os.path.join(PRICE_DATA_DIR, 'nasdaq_tickers.txt')
    
    if not os.path.exists(ticker_list_path):
        print(f"错误：未找到NASDAQ股票列表文件 {ticker_list_path}")
        print("请先运行 prepare_nasdaq_data_polygon.py 生成股票价格数据")
        return
    
    with open(ticker_list_path, 'r') as f:
        nasdaq_tickers = [line.strip() for line in f.readlines() if line.strip()]
    
    print(f"📊 数据集统计:")
    print(f"  - NASDAQ股票总数: {len(nasdaq_tickers)} 只")
    print(f"  - 收集模式: 全量收集 (数据集中所有股票)")
    print(f"  - API优化: 升级版高速模式")
    
    # 显示前10只和最后10只股票作为预览
    print(f"\n📝 股票列表预览:")
    print(f"前10只: {', '.join(nasdaq_tickers[:10])}")
    print(f"最后10只: {', '.join(nasdaq_tickers[-10:])}")
    
    confirm = input(f"\n确认开始收集全部 {len(nasdaq_tickers)} 只NASDAQ股票的新闻数据？(y/n): ").strip().lower()
    if confirm != 'y':
        print("取消收集")
        return
    
    process_tickers = nasdaq_tickers
    ...
    if combined_sentiment is not None:
        # 3. 保存成功处理的股票列表
        success_ticker_path = os.path.join(NEWS_DIR, 'nasdaq_news_tickers.txt')
        with open(success_ticker_path, 'w') as f:
            for ticker in successful_tickers:
                f.write(f"{ticker}\n")
        
        print(f"\n📊 NASDAQ新闻数据收集总结 (升级版高速模式):")
        print(f"  - 数据集总股票数: {len(nasdaq_tickers)}")
        print(f"  - 成功处理股票数: {len(successful_tickers)}")
        print(f"  - 成功率: {len(successful_tickers)/len(nasdaq_tickers)*100:.1f}%")
        print(f"  - 数据保存位置: {NEWS_DIR}")
        print(f"  - 股票列表保存: {success_ticker_path}")
        print(f"  - 合并数据文件: nasdaq_combined_daily_sentiment.csv")
        print(f"  - 收集效率: 升级版API高速优化")
        
    else:
        print("❌ NASDAQ新闻情绪数据收集失败")
